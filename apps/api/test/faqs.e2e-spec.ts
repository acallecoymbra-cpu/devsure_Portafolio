...
describe('Admin faqs (e2e)', () => {
  ...
  function validFaq(overrides: Record<string, unknown> = {}) {
    return {
      question: { en: 'Do you work remotely?', es: '¿Trabajan de forma remota?' },
      answer: { en: 'Yes, fully remote.', es: 'Sí, completamente remoto.' },
      ...overrides,
    };
  }

  it('rejects unauthenticated access', async () => {
    await request(app.getHttpServer()).get('/api/v1/admin/faqs').expect(401);
  });

  it('rejects a faq with an empty answer', async () => {
    await request(app.getHttpServer())
      .post('/api/v1/admin/faqs')
      .set('Origin', ORIGIN)
      .set('Cookie', cookie)
      .set('x-csrf-token', csrfToken)
      .send(validFaq({ answer: {} }))
      .expect(400);
  });
  ...
  it('creates a faq', async () => {
    ...
    expect(response.body).toMatchObject({ question: validFaq().question, sortOrder: 0 });
  });

  it('lists, updates, and deletes the faq end to end', async () => {
    ...
    const updated = await request(app.getHttpServer())
      .patch(`/api/v1/admin/faqs/${created.id}`)
      .set('Origin', ORIGIN)
      .set('Cookie', cookie)
      .set('x-csrf-token', csrfToken)
      .send({ sortOrder: 1 })
    ...
    await request(app.getHttpServer())
      .delete(`/api/v1/admin/faqs/${created.id}`)
      .set('Origin', ORIGIN)
      .set('Cookie', cookie)
      .set('x-csrf-token', csrfToken)
      .expect(204);
    ...
    await request(app.getHttpServer())
      .get(`/api/v1/admin/faqs/${created.id}`)
      .set('Cookie', cookie)
      .expect(404);
  });
});
